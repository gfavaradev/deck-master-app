...
class _AddCardDialogState extends State<_AddCardDialog> {
  ...
  void initState() {
    super.initState();
    nameController = TextEditingController(text: widget.initialCatalogCard?['name'] ?? '');
    serialController = TextEditingController();
    typeController = TextEditingController(text: widget.initialCatalogCard?['type'] ?? '');
    rarityController = TextEditingController();
    quantityController = TextEditingController(text: '1');
    valueController = TextEditingController(text: '0.0');
    descController = TextEditingController(text: widget.initialCatalogCard?['description'] ?? '');
    selectedAlbumId = widget.availableAlbums.first.id;
    selectedCatalogCard = widget.initialCatalogCard;

    if (selectedCatalogCard != null) {
      _updateSets(selectedCatalogCard!['id']);
    }
  }

  Future<void> _updateSets(String cardId) async {
    final sets = await _dbHelper.getCardSets(cardId);
    if (!mounted) return;
    setState(() {
      availableSets = sets;
      if (sets.isNotEmpty) {
        selectedSet = sets.first;
        _applySet(selectedSet!);
      } else {
        selectedSet = null;
      }
    });
  ...
  void _applySet(Map<String, dynamic> set) {
    ...
    valueController.text = set['setPrice'].toString();
  }
            ...
            if (selectedCatalogCard == null)
              TextField(
                ...
                onSubmitted: (query) async {
                  final results = await _dbHelper.getCatalogCards(widget.collectionKey, query: query);
                  if (results.isNotEmpty) {
                    setState(() {
                      selectedCatalogCard = results.first;
                      nameController.text = selectedCatalogCard!['name'];
                      typeController.text = selectedCatalogCard!['type'];
                      descController.text = selectedCatalogCard!['description'];
                    });
                    _updateSets(selectedCatalogCard!['id']);
                  }
                },
              ...
              ListTile(
                ...
                title: Text(selectedCatalogCard!['name'], style: const TextStyle(fontWeight: FontWeight.bold)),
              ...
              DropdownButtonFormField<Map<String, dynamic>>(
                ...
                items: availableSets.map((set) {
                  return DropdownMenuItem<Map<String, dynamic>>(
                    value: set,
                    child: Text('${set['setCode']} - ${set['setRarity']}'),
                  );
                }).toList(),
                onChanged: (val) {
                  setState(() {
                    selectedSet = val;
                    if (val != null) _applySet(val);
                  });
                },
              ),
  ...
  Future<void> _saveCard() async {
    ...
    final cardToSave = CardModel(
      catalogId: selectedCatalogCard?['id'],
      name: name,
